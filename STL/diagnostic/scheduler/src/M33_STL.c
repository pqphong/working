...
#endif
#ifdef M33_STL_TEST_64_PRESENT
#include "m33_stl_cpu_n064.h"
#endif
#ifdef M33_STL_TEST_65_PRESENT
#include "m33_stl_cpu_n065.h"
#endif
#ifdef M33_STL_TEST_66_PRESENT
#include "m33_stl_cpu_n066.h"
#endif
#ifdef M33_STL_TEST_67_PRESENT
#include "m33_stl_cpu_n067.h"
#endif
#ifdef M33_STL_TEST_68_PRESENT
#include "m33_stl_cpu_n068.h"
#endif
#ifdef M33_STL_TEST_69_PRESENT
#include "m33_stl_cpu_n069.h"
#endif
#ifdef M33_STL_TEST_70_PRESENT
#include "m33_stl_cpu_n070.h"
#endif
#ifdef M33_STL_TEST_71_PRESENT
#include "m33_stl_cpu_n071.h"
#endif
#ifdef M33_STL_TEST_72_PRESENT
#include "m33_stl_cpu_n072.h"
#endif
#include "m33_stl_mpu_n000.h"
#include "m33_stl_mpu_n001.h"
#include "m33_stl_nvic_n000.h"

static uint32_t m33_stl_check_bitmask(const uint32_t bitMaskArray[]);

static uint32_t m33_stl_scheduler(const uint32_t bitMaskArray[]);

static uint32_t m33_stl_init_dataStruct(void);

static uint32_t m33_stl_count_tests(const uint32_t bitMaskArray[], uint32_t *count);

static uint32_t m33_stl_mpu_functionality(const uint32_t *addressMPU, uint8_t mpu_region);
...
static uint32_t m33_stl_scheduler(const uint32_t bitMaskArray[]) {
    ...
    uint32_t count = M33_STL_ZERO_VALUE;

    uint32_t id_test = M33_STL_ZERO_VALUE;

    uint32_t i = M33_STL_ZERO_VALUE;

    uint32_t lastI = M33_STL_ZERO_VALUE;
    uint32_t totTest = M33_STL_ZERO_VALUE;

    uint32_t currentBitMask = M33_STL_ZERO_VALUE;

    uint32_t counter = M33_STL_ZERO_VALUE;
    ...
    if (s_m33_stl_dataStruct != M33_STL_NULL) {
        ...
        if (retVal == M33_STL_PASS) {
            ...
            do {
                ...
                if ((currentBitMask & M33_STL_ONE_VALUE) == M33_STL_ONE_VALUE) {
...
#endif
#ifdef M33_STL_TEST_64_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_64:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_64;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n064(s_m33_stl_dataStruct);
                            break;
#endif
#ifdef M33_STL_TEST_65_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_65:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_65;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n065(s_m33_stl_dataStruct);
                            break;
#endif
#ifdef M33_STL_TEST_66_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_66:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_66;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n066(s_m33_stl_dataStruct);
                            break;
#endif
#ifdef M33_STL_TEST_67_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_67:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_67;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n067(s_m33_stl_dataStruct);
                            break;
#endif
#ifdef M33_STL_TEST_68_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_68:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_68;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n068(s_m33_stl_dataStruct);
                            break;
#endif
#ifdef M33_STL_TEST_69_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_69:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_69;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n069(s_m33_stl_dataStruct);
                            break;
#endif
#ifdef M33_STL_TEST_70_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_70:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_70;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n070(s_m33_stl_dataStruct);
                            break;
#endif
#ifdef M33_STL_TEST_71_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_71:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_71;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n071(s_m33_stl_dataStruct);
                            break;
#endif
#ifdef M33_STL_TEST_72_PRESENT
                        case (uint8_t)M33_STL_TEST_ID_72:
                            // Update Test ID field in the dataStruct
                            s_m33_stl_dataStruct->Test_ID = M33_STL_MASK_TEST_ID &
                            M33_STL_TEST_ID_72;
                            // Update the STL Status field in the dataStruct
                            s_m33_stl_dataStruct->STL_Status = M33_STL_MASK_STL_STATUS &
                            M33_STL_STATUS_RUNNING;
                            m33_stl_cpu_n072(s_m33_stl_dataStruct);
                            break;
#endif
                        default:
...
static uint32_t m33_stl_check_bitmask(const uint32_t bitMaskArray[]) {
    ...
    uint8_t i;
    uint32_t mask = M33_STL_ZERO_VALUE;
...
#ifdef M33_STL_TEST_0_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_0 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_1_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_1 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_2_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_2 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_3_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_3 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_4_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_4 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_5_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_5 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_6_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_6 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_7_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_7 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_8_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_8 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_9_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_9 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_10_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_10 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_11_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_11 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_12_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_12 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_13_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_13 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_14_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_14 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_15_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_15 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_16_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_16 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_17_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_17 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_18_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_18 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_19_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_19 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_20_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_20 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_21_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_21 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_22_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_22 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_23_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_23 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_24_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_24 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_25_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_25 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_26_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_26 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_27_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_27 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_28_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_28 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_29_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_29 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_30_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_30 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_31_PRESENT
        compTestsBM_vec[M33_STL_ZERO_VALUE] = compTestsBM_vec[M33_STL_ZERO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_31 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_32_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_32 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_33_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_33 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_34_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_34 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_35_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_35 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_36_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_36 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_37_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_37 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_38_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_38 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_39_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_39 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_40_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_40 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_41_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_41 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_42_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_42 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_43_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_43 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_44_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_44 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_45_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_45 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_46_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_46 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_47_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_47 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_48_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_48 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_49_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_49 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_50_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_50 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_51_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_51 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_52_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_52 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_53_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_53 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_54_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_54 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_55_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_55 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_56_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_56 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_57_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_57 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_58_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_58 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_59_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_59 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_60_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_60 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_61_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_61 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_62_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_62 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_63_PRESENT
        compTestsBM_vec[M33_STL_ONE_VALUE] = compTestsBM_vec[M33_STL_ONE_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_63 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_64_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_64 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_65_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_65 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_66_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_66 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_67_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_67 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_68_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_68 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_69_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_69 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_70_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_70 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_71_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_71 % M33_STL_OFFSET32));
#endif
#ifdef M33_STL_TEST_72_PRESENT
        compTestsBM_vec[M33_STL_TWO_VALUE] = compTestsBM_vec[M33_STL_TWO_VALUE] | ((uint32_t) M33_STL_ONE_VALUE << ((uint32_t) M33_STL_TEST_ID_72 % M33_STL_OFFSET32));
#endif
