...
/*----------------------------------------------------------------------------
  Internal References
 *----------------------------------------------------------------------------*/
extern int __stack_ptr;
extern int _start_T  (void);
 ...
 *----------------------------------------------------------------------------*/
 const VectorTableType IntVectors[496] __attribute__((section(".init"))) = {
  (VectorTableType)(&__stack_ptr),       /*     Initial Stack Pointer */
  Reset_Handler, 
...
{  
  ...
  Init_gregs_8_12();        /* Initialize the value for general register r8-r12*/
  Init_regs_mpu_debug();    /* Initialize the value for MPU, DEBUG registers */
  _start_T();
}
...
{
#pragma asm
  movw  r8,   #0x0000        ; Load lower half of r8 with 0x0000
  movt  r8,   #0x0000        ; Load upper half of r8 with 0x0000

;Copy the value of r8 to r9, r10, r11, and r12
  mov   r9,   r8
  mov   r10,  r8
  mov   r11,  r8
  mov   r12,  r8
...
#pragma asm
  ...
  movw  r4,   #0x0000
  movt  r4,   #0x0000
  ldr   r7,   =0xE000ED94   ; Get the address of MPU_CTRL register
  str   r4,   [r7, #0x0]    ; Initialize the value for MPU_CTRL register
  str   r4,   [r7, #0x4]    ; Initialize the value for MPU_RNR register
  str   r4,   [r7, #0x8]    ; Initialize the value for MPU_RBAR register
  str   r4,   [r7, #0xC]    ; Initialize the value for MPU_RLAR register
  ldr   r6,   =0xE000EDC0   ; Get the address of MPU_MAIR0 register
  str   r4,   [r6, #0x0]    ; Initialize the value for MPU_MAIR0 register
  str   r4,   [r6, #0x4]    ; Initialize the value for MPU_MAIR1 register
  dsb   sy
  isb   sy
  ldr   r5,   =0xE000EDF8   ; Get the address of DCRDR register
  str   r4,   [r5, #0x0]    ; Initialize the value for DCRDR register
  pop   {r4-r7}
#pragma endasm  
}
/*----------------------------------------------------------------------------
