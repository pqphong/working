#----------------------------------------------------------------------------------------------
# function name : execute_stl_cpu_n043
# description   : Run tp name
# input         : tp's name
# output        : None
#----------------------------------------------------------------------------------------------
def execute_stl_cpu_n043(tp_name) :

    setup_debug(tp_name)

    # Start testing for CPU_N043
    log_message("=================================")
    log_message("= Test Case: STL_CPU_N43")
    ...
    # Start testing for TEST ELEMENT 244
    log_message("=    TEST ELEMENT 244:")
    if not check_testpoint(M33_STL_TE244_EXP_VAL1, 1): return False
    if not check_testpoint(M33_STL_TE244_EXP_VAL2, 2): return False

    # Start testing for TEST ELEMENT 324
    log_message("=    TEST ELEMENT 324:")
    if not check_testpoint(M33_STL_TE324_EXP_VAL1, 3): return False
    if not check_testpoint(M33_STL_TE324_EXP_VAL2, 4): return False
