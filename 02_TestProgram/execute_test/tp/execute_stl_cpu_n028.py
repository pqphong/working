#----------------------------------------------------------------------------------------------
# function name : execute_stl_cpu_n028
# description   : Run tp name
# input         : tp's name
# output        : None
#----------------------------------------------------------------------------------------------
def execute_stl_cpu_n028(tp_name) :

    setup_debug(tp_name)

    # Start testing for CPU_N028
    log_message("=================================")
    log_message("= Test Case: STL_CPU_N028")
    ...
    # Start testing for TEST ELEMENT 161
    log_message("=    TEST ELEMENT 161:")
    if not check_testpoint(M33_STL_TE161_EXP_VAL1, 1): return False
    if not check_testpoint(M33_STL_TE161_EXP_VAL2, 2): return False
    if not check_testpoint(M33_STL_TE161_EXP_VAL3, 3): return False

    # Start testing for TEST ELEMENT 162
    log_message("=    TEST ELEMENT 162:")
    if not check_testpoint(M33_STL_TE162_EXP_VAL1, 4): return False
    if not check_testpoint(M33_STL_TE162_EXP_VAL2, 5): return False
