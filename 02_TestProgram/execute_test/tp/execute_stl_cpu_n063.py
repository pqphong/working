#----------------------------------------------------------------------------------------------
# function name : execute_stl_cpu_n063
# description   : Run tp name
# input         : tp's name
# output        : None
#----------------------------------------------------------------------------------------------
def execute_stl_cpu_n063(tp_name) :

    setup_debug(tp_name)

    # Start testing for CPU_N063
    log_message("=================================")
    log_message("= Test Case: STL_CPU_N63")
    ...
    # Start testing for TEST ELEMENT 290
    log_message("=    TEST ELEMENT 290")
    if not check_testpoint(M33_STL_TE290_EXP_VAL1, 1): return False
    if not check_testpoint(M33_STL_TE290_EXP_VAL2, 2): return False
    if not check_testpoint(M33_STL_TE290_EXP_VAL1, 3): return False
    if not check_testpoint(M33_STL_TE290_EXP_VAL3, 4): return False
    if not check_testpoint(M33_STL_TE290_EXP_VAL1, 5): return False
    if not check_testpoint(M33_STL_TE290_EXP_VAL4, 6): return False
    if not check_testpoint(M33_STL_TE290_EXP_VAL1, 7): return False
    if not check_testpoint(M33_STL_TE290_EXP_VAL5, 8): return False
