#----------------------------------------------------------------------------------------------
# function name : execute_stl_cpu_n047
# description   : Run tp name
# input         : tp's name
# output        : None
#----------------------------------------------------------------------------------------------
def execute_stl_cpu_n047(tp_name) :

    setup_debug(tp_name)

    # Start testing for CPU_N047
    log_message("=================================")
    log_message("= Test Case: STL_CPU_N47")
    ...
    # Start testing for TEST ELEMENT 423
    log_message("=    TEST ELEMENT 423")
    if not check_testpoint(M33_STL_TE423_EXP_VAL1, 1): return False
    if not check_testpoint(M33_STL_TE423_EXP_VAL2, 2): return False
    if not check_testpoint(M33_STL_TE423_EXP_VAL1, 3): return False
    if not check_testpoint(M33_STL_TE423_EXP_VAL2, 4): return False

    # Start testing for TEST ELEMENT 424
    log_message("=    TEST ELEMENT 424")
    if not check_testpoint(M33_STL_TE424_EXP_VAL1, 5): return False
    if not check_testpoint(M33_STL_TE424_EXP_VAL2, 6): return False
    if not check_testpoint(M33_STL_TE424_EXP_VAL1, 7): return False
    if not check_testpoint(M33_STL_TE424_EXP_VAL2, 8): return False
