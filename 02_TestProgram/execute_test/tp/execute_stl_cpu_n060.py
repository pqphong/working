#----------------------------------------------------------------------------------------------
# function name : execute_stl_cpu_n060
# description   : Run tp name
# input         : tp's name
# output        : None
#----------------------------------------------------------------------------------------------
def execute_stl_cpu_n060(tp_name) :

    setup_debug(tp_name)

    # Start testing for CPU_N060
    log_message("=================================")
    log_message("= Test Case: STL_CPU_N60")
    ...
    # Start testing for TEST ELEMENT 287
    log_message("=    TEST ELEMENT 287")
    if not check_testpoint(M33_STL_TE287_EXP_VAL1, 1): return False
    if not check_testpoint(M33_STL_TE287_EXP_VAL2, 2): return False
    if not check_testpoint(M33_STL_TE287_EXP_VAL1, 3): return False
    if not check_testpoint(M33_STL_TE287_EXP_VAL3, 4): return False
    if not check_testpoint(M33_STL_TE287_EXP_VAL1, 5): return False
    if not check_testpoint(M33_STL_TE287_EXP_VAL4, 6): return False
    if not check_testpoint(M33_STL_TE287_EXP_VAL5, 7): return False
    if not check_testpoint(M33_STL_TE287_EXP_VAL6, 8): return False
