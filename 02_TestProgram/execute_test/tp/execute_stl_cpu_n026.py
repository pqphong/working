#----------------------------------------------------------------------------------------------
# function name : execute_stl_cpu_n026
# description   : Run tp name
# input         : tp's name
# output        : None
#----------------------------------------------------------------------------------------------
def execute_stl_cpu_n026(tp_name) :

    setup_debug(tp_name)

    # Start testing for CPU_N026
    log_message("=================================")
    log_message("= Test Case: STL_CPU_N026")
    ...
    # Start testing for TEST ELEMENT 155
    log_message("=    TEST ELEMENT 155:")
    if not check_testpoint(M33_STL_TE155_EXP_VAL1, 1): return False
    if not check_testpoint(M33_STL_TE155_EXP_VAL2, 2): return False
    if not check_testpoint(M33_STL_TE155_EXP_VAL3, 3): return False
    if not check_testpoint(M33_STL_TE155_EXP_VAL4, 4): return False

    # Start testing for TEST ELEMENT 157
    log_message("=    TEST ELEMENT 157:")
    if not check_testpoint(M33_STL_TE157_EXP_VAL1, 5): return False
    if not check_testpoint(M33_STL_TE157_EXP_VAL2, 6): return False
    if not check_testpoint(M33_STL_TE157_EXP_VAL3, 7): return False
