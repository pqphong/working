#----------------------------------------------------------------------------------------------
# function name : execute_stl_cpu_n055
# description   : Run tp name
# input         : tp's name
# output        : None
#----------------------------------------------------------------------------------------------
def execute_stl_cpu_n055(tp_name) :

    setup_debug(tp_name)

    # Start testing for CPU_N055
    log_message("=================================")
    log_message("= Test Case: STL_CPU_N55")
    ...
    # Start testing for TEST ELEMENT 333
    log_message("=    TEST ELEMENT 333")
    if not check_testpoint(M33_STL_TE333_EXP_VAL1, 1): return False
    if not check_testpoint(M33_STL_TE333_EXP_VAL2, 2): return False
    if not check_testpoint(M33_STL_TE333_EXP_VAL3, 3): return False
    if not check_testpoint(M33_STL_TE333_EXP_VAL4, 4): return False
    if not check_testpoint(M33_STL_TE333_EXP_VAL5, 5): return False
    if not check_testpoint(M33_STL_TE333_EXP_VAL6, 6): return False
